...
class AxisLayer(Module):
    def __init__(self):
        ...
        self.joints_mapping = [5, 6, 7, 9, 10,
                               11, 17, 18, 19, 13, 14, 15, 1, 2, 3]
        up_axis_base = np.vstack((np.array([[0, 1, 0]]).repeat(
            12, axis=0), np.array([[1, 1, 1]]).repeat(3, axis=0)))
        self.register_buffer("up_axis_base", torch.from_numpy(
            up_axis_base).float().unsqueeze(0))
    ...
    def forward(self, hand_joints, transf):
        ...
        b_axis = hand_joints[:, self.joints_mapping] - \
            hand_joints[:, [i + 1 for i in self.joints_mapping]]
        b_axis = (transf[:, 1:, :3, :3].transpose(2, 3)
                  @ b_axis.unsqueeze(-1)).squeeze(-1)
